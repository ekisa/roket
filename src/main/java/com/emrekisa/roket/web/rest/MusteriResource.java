...
public class MusteriResource {
    ...
    @GetMapping("/musteris")
    @Timed
    public ResponseEntity<List<MusteriDTO>> getAllMusteris(Pageable pageable) {
        log.debug("REST request to get a page of Musteris");
        Page<MusteriDTO> page = musteriService.findAll(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/musteris");
